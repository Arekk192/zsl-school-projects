...
def addSubject():
    ...
    if subjectForm.validate_on_submit():
        ...
        with open('data/grades.json', 'w', encoding='utf-8') as gradesFile:
            ...
            return redirect('dashboard')
    return render_template('add-subject.html', title='Dodaj przedmiot', subjectForm=subjectForm, userLogin=session.get('userLogin'), firstName=session.get('firstName'), date=date)
